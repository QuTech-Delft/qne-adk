...
class RemoteApi:
    def __init__(self, config_manager: ConfigManager) -> None:
        ...
        self.auth_manager:AuthManager = AuthManager(self.__config_manager.get_config_dir(), self.__login_user, self.__login_anonymous)
    ...
    def list_applications(self) -> List[ApplicationType]:
        ...
        response = self._action('listApplications', {'token': token})
        return []

    def delete_application(self, application: str) -> None:
        app = self.__config_manager.get_application(application)
        token = self.auth_manager.load_token()
        response = self._action('delete', {'token': token, 'id': 'app_id_int'})
    ...
    def upload_application(self, application: str) -> None:
        ...
        if not application_exists:
            application_id = self.__create_application(application)
    ...
    def __create_application(self, token: str, application: str) -> int:
        response = self._action('createApplication', {'token': token, 'body': 'application_body'})
        return 0

    def __create_application_version(self, token: str, application_id: int) -> int:
        response = self._action('createApplicationVersion', {'token': token, 'id': application_id})
        return 0

    def __create_application_config(self, token: str, application_version_id: int) -> None:
        response = self._action('createApplicationConfig', {'token': token, 'id': application_version_id})

    def __create_application_source(self, token: str, application_version_id: int) -> None:
        response = self._action('createApplicationSource', {'token': token, 'id': application_version_id})

    def __create_application_result(self, token: str, application_version_id: int) -> None:
        response = self._action('createApplicationResult', {'token': token, 'id': application_version_id})

    def publish_application(self, application: str) -> None:
        application_exists, application_id = self.__config_manager.remote_application_exists(application)
        if application_exists:
            token = self.auth_manager.load_token()
            response = self._action('publishApplication', {'token': token, 'id': application_id})
        else:
    ...
    def get_application_config(self, application: str) -> AppConfigType:
        ...
        if application_exists:
            ...
            pass # Application does not exist on remote
    ...
    def delete_experiment(self, path: Path) -> None:
        ...
        if experiment_exists:
            token = self.auth_manager.load_token()
            response = self._action('deleteExperiment', {'token': token, 'id': experiment_id})

    def list_experiments(self) -> List[ExperimentType]:
        token = self.auth_manager.load_token()
        response = self._action('listExperiments', {'token': token})
        return []

    def run_experiment(self, block: bool) -> Optional[List[ResultType]]:
        token = self.auth_manager.load_token()
        response_experiment = self._action('createExperiment', {'token': token, 'body': 'body'})
        response_roundset = self._action('createRoundSet', {'token': token, 'body': 'body'})
    ...
    def get_results(
        ...
        if self.__results_exists(path):
            return self.__get_local_results()
        else:
            token = self.auth_manager.load_token()
            return self._action('getResults', {'token': token, 'body': 'body'})
    ...
    def _action(self, operation_name, params) -> Any:
