...
class TestConfigManager(unittest.TestCase):
    ...
    def test_get_application_from_path(self):
        with patch.object(ConfigManager, "check_config_exists") as check_config_exists, \
             patch("cli.managers.config_manager.read_json_file") as read_json_file_mock:

            read_json_file_mock.return_value = {self.application: {"path": str(self.path) + "/"}}
            check_config_exists.return_value = True

            self.config_manager.get_application_from_path(self.path)

            check_config_exists.assert_called()
            read_json_file_mock.assert_called_once_with(self.config_manager.applications_config)

            # Raise NoApplicationExists when read_json_file is empty
            read_json_file_mock.reset_mock()
            read_json_file_mock.return_value = {}

            self.assertRaises(NoApplicationExists, self.config_manager.get_application_from_path, self.path)
            ...
            check_config_exists.reset_mock()
            check_config_exists.return_value = False
            self.assertRaises(NoConfigFileExists, self.config_manager.get_application_from_path, self.path)
            check_config_exists.assert_called_once()
